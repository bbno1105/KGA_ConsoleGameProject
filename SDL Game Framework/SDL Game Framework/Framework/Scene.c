...
{
    Text    GuideLine[50][20]; //[id][¡ŸπŸ≤ﬁ]
    Text    GuideLineMovingPage[3][20]; //[º±≈√¡ˆ3∞≥][º±≈√¡ˆ±€¿⁄∞≥ºˆ]
    Text    ShadedSelect[3][20]; //[º±≈√¡ˆ3∞≥][º±≈√¡ˆ±€¿⁄∞≥ºˆ]
    int32   FontSize;
    int32   RenderMode;
...
void init_title(void)
{
	...
	wchar_t* testtext = ParseToUnicode(csvFile.Items[44][Text_s]); // csvFile.Items[ID+1][ƒ√∑≥∏Ì]

	for (int32 i = 0; i < 20; ++i)
	{
		wchar_t stringl[100] = L"";
		testtext = StringLine(testtext, stringl); // ø©±‚º≠ Æc
		Text_CreateText(&data->GuideLine[i], "d2coding.ttf", 16, stringl, wcslen(stringl));
		if (*testtext == NULL) break;
	}

    data->ID = 1;               // ID 1∫Œ≈Õ Ω√¿€
    data->TextLine = 0;         // IDæ»¿« ≈ÿΩ∫∆Æ ¡Ÿ 0∫Œ≈Õ Ω√¿€
    data->FontSize = 18;        // µ•¿Ã≈Õ ∆˘∆Æ ªÁ¿Ã¡Ó º≥¡§
    data->RenderMode = SOLID;   // ∑£¥ı∏µÂ : ±€¿⁄∏∏ ≥™ø¿∞‘

    // ID 1∫Œ≈Õ ≈ÿΩ∫∆Æ 20¡Ÿ±Ó¡ˆ ¿Ã¥÷
    for (int32 i = 0; i < 20; ++i)
    {
